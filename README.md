...
- **新增参考读取规划：生成回答前先让模型挑选最相关的页面与参考图片，再流式生成主回复，长资料不再一股脑塞进上下文！！！**
...
- **按需读取参考**：生成主回复前，会先基于用户问题和参考地图规划需要阅读的页面与图片，再把筛选后的上下文交给主模型；当规划失败时会自动使用预算内兜底上下文，避免直接卡死。
...
- **位置提问**：可在回答的具体位置发起小窗问答，支持多轮追问，并可保存为正文中的位置标记；位置提问也会复用按需读取参考的流程，尽量只带入与当前位置相关的资料。
