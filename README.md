...
MindLinker 是一个面向课程、论文和长文档阅读的开源桌面学习助手。它支持导入本地资料，围绕资料生成讲解，并把概念解释、位置提问和后续追问保留在对应的阅读位置，帮助用户在阅读过程中逐步建立自己的知识地图。

它是本地运行的开源桌面应用，基于 Electron、React、TypeScript 和 Vite 构建。

## 为什么做它

使用传统 AI 软件学习课程、论文或讲义时，常见体验是：看到中间某个概念不懂，只能一路跳到对话最后提问；问完再翻回原文，翻着翻着又忘了刚才看到哪。超长对话窗口越滚越长，思路越滚越散，问题还没解决，人先在上下文里迷路了。

于是 MindLinker 来了：哪里不懂点哪里，哪里想问就在原地开聊。解释可以贴在词上，追问可以留在位置上，参考资料、主回复、解释链和知识图谱一起长成一张学习地图。少一点来回翻动，多一点“我刚想到的问题就应该出现在这里”的顺手感。
...
> [!NOTE]
> 部分功能的实际能力会直接受所使用模型能力影响。尤其是 PDF 页面图片理解、截图理解、复杂公式/表格解释、长上下文阅读和关键词抽取等能力，需要模型本身具备足够好的视觉、多模态、长上下文和结构化输出能力。

> [!TIP]
> **Wanted:** 如果你对本地向量库、检索增强生成、知识图谱布局、概念抽取或学习型交互有改进想法，欢迎提交 Issue 或 Pull Request。尤其欢迎能复现真实学习场景问题的反馈、测试用例和小而清晰的修复提交。

## 最近更新

### 2026-05-29

- **想找某个概念在哪页？PDF 文本搜索工具上线，关键词、原文片段、页码一起交给模型！！！**
- **资料太多塞不下？先挑重点再回答，长 PDF 也能认真读！！！**
- **支持模型在输出中引用并展示参考中的图片，数据图表不错过！！！**

## 功能特性

- **项目化工作区**：以学习项目为最上级目录，每个项目下管理参考资料和多个对话。
- **参考驱动对话**：支持导入 PDF、Markdown、文本等资料，本地解析后作为模型上下文参与回答。
- **按需读取参考**：资料很多也不用一股脑全塞给模型，会先挑出更相关的页面和图表，再组织回答；挑不准时也会自动兜底，不让对话卡在半路。
- **PDF 文本搜索工具**：模型可以先按关键词检索 PDF 已提取文本，拿到原文片段和页码标记后再决定读哪里；如果 PDF 没有可检索文本，也会明确知道它可能是扫描件或图片型资料。
...
- **位置提问**：哪里想问就在哪里开聊，支持多轮追问，还能把这段小对话留在原位置；追问时也会尽量只翻和当前位置有关的资料。
...
![可导入多个参考，哪怕是课程所有ppt也不怕](<docs/images/可导入多个参考，哪怕是课程所有ppt也不怕.png>)

### 模型回复看不懂？一键划词解释，全局链接一劳永逸

![模型回复看不懂？一键划词解释，全局链接一劳永逸](<docs/images/模型回复看不懂？一键划词解释，全局链接一劳永逸.png>)

### 好多词语都不会？一键智能划词，让ai帮你划出关键词（猜你想问

![好多词语都不会？一键智能划词，让ai帮你划出关键词（猜你想问](<docs/images/好多词语都不会？一键智能划词，让ai帮你划出关键词（猜你想问.png>)

### 模型的解释还是看不懂？别怕，一问到底，一直问到懂！！

![模型的解释还是看不懂？别怕，一问到底，一直问到懂！！](<docs/images/模型的解释还是看不懂？别怕，一问到底，一直问到懂！！.png>)

### 想要探讨？直接开聊不等待，聊完还能打下标记，下次回来接着聊！！！

![想要探讨？直接开聊不等待，聊完还能打下标记，下次回来接着聊！！！](<docs/images/想要探讨？直接开聊不等待，聊完还能打下标记，下次回来接着聊！！！.png>)
...
- `npm run export:interactions`：导出模型互动与本地解析报告，默认整理最近一个对话，输出包含 `report.md` 和 `assets/` 的文件夹。
- `npm run inspect:reference`：解析单个参考文件，并导出应用可见的参考结构报告文件夹。

调试模型阅读流程时，可以运行：

```bash
npm run export:interactions -- --out temp/latest-interaction
```

这个工具不属于应用界面，只用于开发和问题复盘。它会读取 Electron 本地运行日志，并尽量拼接本地项目、对话、参考解析结果、位置提问和模型回复。默认会在输出目录中写入 `report.md`，并把报告中引用到的图片保存到 `assets/` 后用 Markdown 引用；如果传入 `.md` 路径，仍会使用旧的单文件报告模式，并把图片写到相邻的 `*-assets/` 文件夹。需要查看全部记录时可加 `--all`；需要指定范围时可使用 `--conversation-id`、`--project-id`、`--since` 或 `--message`。

查看某个参考文件解析后的样子，可以运行：

```bash
npm run inspect:reference -- /path/to/reference.pdf --out temp/reference-report
```

该工具会展示页级文本、图片输入占位、可引用图片资产、模型上下文 XML 和 OpenAI input parts 预览。默认输出为文件夹，图片会落到 `assets/` 并在 `report.md` 中以相对路径引用，打开 Markdown 时更直观。它在 Node 环境下运行，PDF 页面截图没有浏览器 canvas 支持时会显示占位图；应用内真实导入仍会按桌面窗口环境尝试渲染页面图片。
