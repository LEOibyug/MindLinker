...
- **参考图片引用**：模型回复现在可以通过 `[[ref-image:图片ID]]` 或 `<ref-image id="图片ID" />` 引用参考资料解析出的图片，并在阅读器正文中渲染出来。可引用图片只来自参考解析结果中的图片资产；PDF 页截图仍只作为多模态输入给模型，不会被当作可引用图片。
- **纯文本参考支持**：新增 Markdown、TXT 等纯文本文件支持。导入这类参考时会直接读取文件正文，并带上参考名称发送给模型，不再只使用占位符。
- **提示词协议补充**：主回复提示词已加入参考图片引用规则，要求模型只引用明确列出的 `<REFERENCE_IMAGE>`，避免编造图片 id 或引用 PDF 页面截图。
